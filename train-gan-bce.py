...
class Trainer:
    def __init__(
        ...
        self.recon_crit = nn.BCELoss(reduction="sum")
    ...
    def _save_checkpoint(self, epoch: int):
        ...
        model_path = f"./weights/generator-bce_{epoch}.pt"
        torch.save(ckp, model_path)

        ckp = self.d_l.module.state_dict()
        model_path = f"./weights/d_l-bce_{epoch}.pt"
        torch.save(ckp, model_path)

        ckp = self.d_h.module.state_dict()
        model_path = f"./weights/d_h-bce_{epoch}.pt"
        torch.save(ckp, model_path)
    ...
    def update_g(self, low_imgs, high_imgs):
        ...
        loss_h = self.adv_crit(
            fake_pred, fake_labels
        ) + hyperparameters.beta * self.recon_crit(fake_batch, high_imgs)
        loss_h.backward()
        self.optim_g.step()
    ...
    def train(self, max_epoch):
        ...
        for epoch in range(max_epoch):
            ...
            if epoch % hyperparameters.ckpt_per == 0 and self.gpu_id == 0:
                with open("dl_losses-bce.pkl", mode="wb") as file:
                    pickle.dump(dl_losses, file, pickle.HIGHEST_PROTOCOL)
                with open("dh_losses-bce.pkl", mode="wb") as file:
                    pickle.dump(dh_losses, file, pickle.HIGHEST_PROTOCOL)
                with open("g_losses-bce.pkl", mode="wb") as file:
