...
class Trainer:
    ...
    def _train_generator(self, batch):
        ...
        for _ in range(hyperparameters.max_iter):
            ...
            target_labels = torch.full(pred_labels.shape, 1, dtype=torch.long, device=self.gpu_id)
    ...
    def _train_discriminator(self, batch, gen_imgs):
        ...
        for _ in range(hyperparameters.max_iter):
            for imgs, label in zip([low_imgs, high_imgs, gen_imgs], [1, 1, 0]):
                ...
                target_labels = torch.full(pred_labels.shape, label, dtype=torch.long, device=self.gpu_id)
